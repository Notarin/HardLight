...
public sealed class EmbedTest : InteractionTest
{
    ...
    public async Task TestDisconnectWhileEmbedded()
    {
        ...
        Assert.That(embedComp.EmbeddedIntoUid, Is.EqualTo(ToServer(Target)), "Projectile not embedded into target");
    ...
    public async Task TestDeleteWhileEmbedded()
    {
        ...
        {
            Assert.That(!SEntMan.Deleted(embedded), "Target EmbeddedObjects contains a deleted entity.");
        }
        Assert.That(
            containerComp.EmbeddedObjects,
            Has.Count.EqualTo(1),
