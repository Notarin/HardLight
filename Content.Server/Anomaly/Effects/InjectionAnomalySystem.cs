...
public sealed class InjectionAnomalySystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<InjectionAnomalyComponent, AnomalySupercriticalEvent>(
            OnSupercritical,
    ...
    private void OnPulse(Entity<InjectionAnomalyComponent> entity, ref AnomalyPulseEvent args)
    {
        PulseScalableEffect(
            entity,
            entity.Comp.InjectRadius * args.PowerModifier,
            entity.Comp.MaxSolutionInjection * args.Severity * args.PowerModifier
        );
    }
    ...
    private void OnSupercritical(Entity<InjectionAnomalyComponent> entity, ref AnomalySupercriticalEvent args)
    {
        PulseScalableEffect(
            entity,
            entity.Comp.SuperCriticalInjectRadius * args.PowerModifier,
            entity.Comp.SuperCriticalSolutionInjection * args.PowerModifier
        );
    }
    ...
    private void PulseScalableEffect(Entity<InjectionAnomalyComponent> entity, float injectRadius, float maxInject)
    {
        ...
        var allEnts = _lookup
            .GetEntitiesInRange<InjectableSolutionComponent>(
                _transform.GetMapCoordinates(entity, xform: xform),
                injectRadius
            )
            .Select(x => x.Owner)
            .ToList();
