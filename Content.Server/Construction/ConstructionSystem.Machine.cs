...
public sealed partial class ConstructionSystem
{
    ...
    [Dependency]
    private readonly BindToStationSystem _bindToStation = default!; // Frontier
        ...
        foreach (var (part, amount) in machineBoard.Requirements)
        {
            ...
            for (var i = 0; i < amount; i++)
            {
                ...
                if (!_container.Insert(p, partContainer))
                    throw new Exception(
                        $"Couldn't insert machine part of type {part} to machine with prototype {partProto.StockPartPrototype.ToString() ?? "N/A"}!"
                    );
            }
        }
