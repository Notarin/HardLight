...
public sealed partial class GeneticAnalyzerWindow : FancyWindow
{
    ...
    private static readonly VectorFont FontBold = new(
        IoCManager
            ...
            .GetResource<FontResource>("/Fonts/NotoSansDisplay/NotoSansDisplay-Bold.ttf"),
        14
    );
    ...
    private static readonly VectorFont FontRegular = new(
        IoCManager
            .Resolve<IResourceCache>()
            .GetResource<FontResource>("/Fonts/NotoSansDisplay/NotoSansDisplay-Regular.ttf"),
        14
    );
    ...
    public void Populate(GeneticAnalyzerUiState state)
    {
        ...
        var sortedMutations = state.Mutations?.Where(m => m.Block > 0).OrderBy(m => m.Block).ToList() ?? new();

        if (!sortedMutations.Any())
        {
            var noBlocksLabel = new Label { Text = Loc.GetString("genetic-analyzer-no-enzymes") };
