...
public sealed class PlumbingReactorSystem : EntitySystem
{
    ...
    private void OnReactorUpdate(Entity<PlumbingReactorComponent> ent, ref PlumbingDeviceUpdateEvent args)
    {
        ...
        if (allMet)
        {
            ...
            bool toggled = false;
            if (_solutionSystem.TryGetSolution(ent.Owner, ent.Comp.BufferSolutionName, out var bufferSolution))
            {
                bufferSolution.Value.Comp.Solution.CanReact = true;
                toggled = true;
            }
            ...
            if (products.Count > 0)
            {
                ...
                {
                    var removed = _solutionSystem.RemoveReagentAndReturn(bufferEnt.Value, reagent, quantity);
                    if (removed > 0)
            ...
            if (toggled)
            {
                bufferSolution!.Value.Comp.Solution.CanReact = false;
            }
        }
