...
namespace Content.Server.Administration.Systems
{
    ...
    {
        [Dependency]
        private readonly IConGroupController _groupController = default!;
        ...
        [Dependency]
        private readonly DisposalTubeSystem _disposalTubes = default!;

        [Dependency]
        private readonly EuiManager _euiManager = default!;

        [Dependency]
        private readonly GhostRoleSystem _ghostRoleSystem = default!;

        [Dependency]
        private readonly UserInterfaceSystem _uiSystem = default!;

        [Dependency]
        private readonly PrayerSystem _prayerSystem = default!;
        ...
        [Dependency]
        private readonly RejuvenateSystem _rejuvenate = default!;
        ...
        [Dependency]
        private readonly StationSpawningSystem _spawning = default!;
        ...
        [Dependency]
        private readonly AdminFrozenSystem _freeze = default!;

        [Dependency]
        private readonly IPlayerManager _playerManager = default!;

        [Dependency]
        private readonly SiliconLawSystem _siliconLawSystem = default!;
        ...
        private void AddAdminVerbs(GetVerbsEvent<Verb> args)
        {
            ...
            if (_adminManager.IsAdmin(player))
            {
                ...
                mark.Act = () =>
                    _toolshed.InvokeCommand(player, "=> $marked", new List<EntityUid> { args.Target }, out _);
                ...
                if (TryComp(args.Target, out ActorComponent? targetActor))
                {
                    ...
                    crypticVerb.Act = () =>
                    {
                        _quickDialog.OpenDialog(
                            player,
                            "Subtle Cryptic Message",
                            "Popup Message",
                            "Scale (0.5-3.0)",
                            (string popupMessage, float scale) =>
                            {
                                scale = Math.Clamp(scale, 0.5f, 3.0f);
                                var scaledMessage = $"[scale:{scale:F1}]{popupMessage}";
                                _prayerSystem.SendCrypticMessage(
                                    targetActor.PlayerSession,
                                    player,
                                    "",
                                    scaledMessage == "" ? Loc.GetString("prayer-popup-cryptic-default") : scaledMessage
                                );
                            }
                        );
                    };
                    ...
                    args.Verbs.Add(
                        new Verb()
                        {
                            ...
                            Act = () =>
                            {
                                ...
                                {
                                    _popup.PopupEntity(
                                        Loc.GetString("admin-player-spawn-failed"),
                                        args.User,
                                        args.User
                                    );
                                    return;
                                ...
                                var mobUid = _spawning.SpawnPlayerMob(
                                    coords.Value,
                                    null,
                                    profile,
                                    stationUid,
                                    session: targetActor.PlayerSession
                        ...
                        new Verb()
                        {
                            ...
                            Act = () =>
                            {
                                ...
                                var mobUid = _spawning.SpawnPlayerMob(
                                    ...
                                    stationUid,
                                    session: targetActor.PlayerSession
                                ); // Frontier: Added session, // HardLight: Added var mobUid =
                                _traits.ApplyProfileTraits(mobUid, profile, targetActor.PlayerSession.Name); // HardLight
                            },
                            ConfirmationPopup = true,
                            Impact = LogImpact.High,
                    ...
                    args.Verbs.Add(
                        new Verb
                        {
                            ...
                            Category = VerbCategory.Admin,
                            Act = () =>
                                _console.ExecuteCommand(player, $"playerpanel \"{targetActor.PlayerSession.UserId}\""),
                ...
                if (_mindSystem.TryGetMind(args.Target, out var mindId, out var mindComp) && mindComp.UserId != null)
                {
                    ...
                    args.Verbs.Add(
                        new Verb
                        {
                            ...
                            Icon = new SpriteSpecifier.Texture(
                                new("/Textures/Interface/VerbIcons/sentient.svg.192dpi.png")
                            ),
                ...
                if (!frozen)
                {
                    args.Verbs.Add(
                        new Verb
                        {
                            ...
                            Category = VerbCategory.Admin,
                            Icon = new SpriteSpecifier.Texture(
                                new("/Textures/Interface/VerbIcons/snow.svg.192dpi.png")
                            ),
                            Act = () =>
                            {
                            ...
                            if (HasComp<MapGridComponent>(args.Target))
                            {
                                if (player.AttachedEntity != null)
                                {
                                    ...
                                    _console.ExecuteCommand(
                                        player,
                                        $"tpgrid {GetNetEntity(args.Target)} {mapPos.X} {mapPos.Y} {mapPos.MapId}"
                                    );
                                }
                            }
                            else
                ...
                // When inspecting the core we can find the entity with its laws by looking at the  AiHolderComponent.
                    ...
                    TryComp<StationAiHolderComponent>(args.Target, out var holder)
                ...
                )
                {
                    ...
                    TryComp<MovementRelayTargetComponent>(args.Target, out var relay)
                    ...
                    args.Verbs.Add(
                        new Verb()
                        {
                            ...
                            Icon = new SpriteSpecifier.Rsi(
                                new ResPath("/Textures/Interface/Actions/actions_borg.rsi"),
                                "state-laws"
                            ),
                        }
                    );
        ...
        private void AddDebugVerbs(GetVerbsEvent<Verb> args)
        {
            ...
            // Delete verb
            if (
                _toolshed.ActivePermissionController?.CheckInvokable(
                    new CommandSpec(_toolshed.DefaultEnvironment.GetCommand("delete"), null),
                    player,
                    out _
                ) ?? false
            )
            ...
            if (
                _toolshed.ActivePermissionController?.CheckInvokable(
                    new CommandSpec(_toolshed.DefaultEnvironment.GetCommand("rejuvenate"), null),
                    player,
                    out _
                ) ?? false
            )
            ...
            if (
                _toolshed.ActivePermissionController?.CheckInvokable(
                    new CommandSpec(_toolshed.DefaultEnvironment.GetCommand("mind"), "control"),
                    player,
                    out _
                )
                ?? false && args.User != args.Target
            )
            {
                Verb verb = new()
                {
            ...
            // Make Sentient verb
            if (
                _groupController.CanCommand(player, "makesentient")
            ...
            // Set clothing verb
            if (
                _groupController.CanCommand(player, "setoutfit")
            ...
            if (
                _groupController.CanCommand(player, "tubeconnections")
            ...
            if (
                _groupController.CanCommand(player, "makeghostrole")
            ...
            if (
                _groupController.CanAdminMenu(player)
            ...
            if (
                _groupController.CanCommand(player, "addreagent")
