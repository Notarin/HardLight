...
public sealed partial class BankSystem
{
    ...
    private void OnWithdraw(EntityUid uid, StationBankATMComponent component, StationBankWithdrawMessage args)
    {
        ...
        if (!TryGetBalance(component.Account, out var stationBank))
        {
            ...
            _uiSystem.SetUiState(uid, args.UiKey, new StationBankATMMenuInterfaceState(0, false, deposit));
            return;
        }
        ...
        _adminLogger.Add(
            LogType.ATMUsage,
            LogImpact.Low,
            $"{ToPrettyString(player):actor} withdrew {args.Amount} from {component.Account} station bank account. '{args.Reason}': {args.Description}"
        );
    ...
    private void OnDeposit(EntityUid uid, StationBankATMComponent component, StationBankDepositMessage args)
    {
        ...
        if (component.CashSlot.ContainerSlot is not BaseContainer cashSlot)
        {
            ...
            ConsolePopup(args.Actor, Loc.GetString("bank-atm-menu-no-bank"));
            PlayDenySound(uid, component);
            _uiSystem.SetUiState(uid, args.UiKey, new StationBankATMMenuInterfaceState(stationBank, false, deposit));
            return;
        }
        ...
        if (
            !TryComp<StackComponent>(component.CashSlot.ContainerSlot.ContainedEntity, out var stackComponent)
        ...
        if (
            _prototypeManager.Index(component.CashType)
            != _prototypeManager.Index<StackPrototype>(stackComponent.StackTypeId)
        )
        {
        ...
        _adminLogger.Add(
            LogType.ATMUsage,
            LogImpact.Low,
            $"{ToPrettyString(player):actor} deposited {args.Amount} to {component.Account} station bank account. '{args.Reason}': {args.Description}"
        );
    ...
    private void SetInsertedCashAmount(
        StationBankATMComponent component,
        int amount,
        out int leftAmount,
        out bool empty
    )
