    ...
    public void CreateCurrencyButtons(Dictionary<ProtoId<CurrencyPrototype>, FixedPoint2> balance)
    {
        ...
        foreach (var currency in _validCurrencies)
        {
            ...
            var button = new CurrencyWithdrawButton()
            {
                ...
                MinHeight = 20,
                Text = Loc.GetString(
                    "store-withdraw-button-ui",
                    (
                        "currency",
                        Loc.GetString(
                            currency.Key.DisplayName,
                            ("amount", BankSystemExtensions.ToIndependentString((int)currency.Value))
                        )
                    )
                ), // Frontier: currency.Value -> BankSystemExtensions.GetIndendependentString((int) currency.Value)
            };
