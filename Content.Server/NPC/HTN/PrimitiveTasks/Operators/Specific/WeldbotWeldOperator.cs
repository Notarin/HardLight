...
public sealed partial class WeldbotWeldOperator : HTNOperator
{
    ...
    public override HTNOperatorStatus Update(NPCBlackboard blackboard, float frameTime)
    {
        ...
        if (
            (!weldableIsSilicon && !weldableIsStructure)
        ...
        if (botComp.IsEmagged)
        {
            ...
            _damageableSystem.TryChangeDamage(
                target,
                new DamageSpecifier(prototype, EmaggedBurnDamage),
                true,
                false,
                damage
            );
        ...
        else
        {
            if (weldableIsSilicon)
            {
                ...
                _damageableSystem.TryChangeDamage(
                    target,
                    new DamageSpecifier(prototype, -SiliconRepairAmount),
                    true,
                    false,
                    damage
                );
        ...
        if (
            (weldableIsSilicon && damage.DamagePerGroup["Brute"].Value == 0)
            || (weldableIsStructure && damage.TotalDamage.Value == 0)
        )
            ; //only say "all done if we're actually done!"
