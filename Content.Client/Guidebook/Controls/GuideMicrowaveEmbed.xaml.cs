        ...
        IPrototypeRepresentationControl
{
    ...
    private void GenerateSolidIngredients(FoodRecipePrototype recipe)
    {
        foreach (var (product, amount) in recipe.IngredientsSolids.OrderByDescending(p => p.Value))
        {
            ...
            solidNameMsg.AddMarkupOrThrow(
                Loc.GetString("guidebook-microwave-solid-name-display", ("ingredient", ingredient.Name))
            );
            solidNameMsg.Pop();
            ...
            solidQuantityMsg.AddMarkupOrThrow(
                Loc.GetString("guidebook-microwave-solid-quantity-display", ("amount", amount))
            );
    ...
    private void GenerateLiquidIngredients(FoodRecipePrototype recipe)
    {
        foreach (var (product, amount) in recipe.IngredientsReagents.OrderByDescending(p => p.Value))
        {
            ...
            liquidColorMsg.AddMarkupOrThrow(
                Loc.GetString("guidebook-microwave-reagent-color-display", ("color", reagent.SubstanceColor))
            );
            liquidColorMsg.Pop();
            ...
            liquidNameMsg.AddMarkupOrThrow(
                Loc.GetString("guidebook-microwave-reagent-name-display", ("reagent", reagent.LocalizedName))
            );
            liquidNameMsg.Pop();
            ...
            liquidQuantityMsg.AddMarkupOrThrow(
                Loc.GetString("guidebook-microwave-reagent-quantity-display", ("amount", amount))
            );
    ...
    private void GenerateCookTime(FoodRecipePrototype recipe)
    {
        ...
        msg.AddMarkupOrThrow(
            Loc.GetString(
                ...
                ("processingTypes", processingTypeString),
