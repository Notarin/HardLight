...
public sealed class VaporVisualizerSystem : VisualizerSystem<VaporVisualsComponent>
{
    ...
    private void OnComponentInit(EntityUid uid, VaporVisualsComponent comp, ComponentInit args)
    {
        ...
        if (
            AppearanceSystem.TryGetData<bool>(uid, VaporVisuals.State, out var state)
            && state
    ...
    {
        if (
            AppearanceSystem.TryGetData<Color>(uid, VaporVisuals.Color, out var color, args.Component)
            && args.Sprite != null
        )
        {
