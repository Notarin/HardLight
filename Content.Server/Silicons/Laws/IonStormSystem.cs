...
public sealed class IonStormSystem : EntitySystem
{
    ...
    [Dependency]
    private readonly SiliconLawSystem _siliconLaw = default!;
    ...
    public void IonStormTarget(Entity<SiliconLawBoundComponent, IonStormTargetComponent> ent, bool adminlog = true)
    {
        ...
        {
            var i = _robustRandom.Next(laws.Laws.Count);
            laws.Laws[i] = new SiliconLaw() { LawString = newLaw, Order = laws.Laws[i].Order };
        }
        else
        {
            laws.Laws.Insert(
    ...
    private string GenerateLaw()
    {
        ...
        return _robustRandom.Next(0, 35) switch
        {
            ...
            14 => Loc.GetString(
                "ion-storm-law-feeling-subjects",
                ("who", who),
                ("feeling", feeling),
                ("joined", joined),
                ("subjects", triple)
            ...
            28 => Loc.GetString(
                "ion-storm-law-crew-only-subjects",
                ("adjective", adjective),
                ("subjects", subjects),
                ("part", part)
            ),
            ...
            30 => Loc.GetString(
                "ion-storm-law-crew-must-have",
                ("adjective", adjective),
                ("objects", objects),
                ("part", part)
            ),
            31 => Loc.GetString(
                "ion-storm-law-crew-must-eat",
                ("who", who),
                ("adjective", adjective),
                ("food", food),
                ("part", part)
