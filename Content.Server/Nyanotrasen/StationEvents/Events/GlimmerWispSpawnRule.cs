...
internal sealed class GlimmerWispRule : StationEventSystem<GlimmerWispRuleComponent>
{
    ...
    protected override void Started(
        EntityUid uid,
        GlimmerWispRuleComponent component,
    ...
    )
    {
        ...
        var normalSpawnLocations = EntityManager
            .EntityQuery<VentCritterSpawnLocationComponent, TransformComponent>()
            .ToList();
        var hiddenSpawnLocations = EntityManager
            .EntityQuery<MidRoundAntagSpawnLocationComponent, TransformComponent>()
            .ToList();

        var baseCount = Math.Max(1, EntityManager.EntityQuery<PsionicComponent>().Count() / 10);
