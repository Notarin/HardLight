...
public sealed partial class BanPanel : DefaultWindow
{
    ...
    private void CreateRoleGroup(string roleName, IEnumerable<string> roleList, Color color)
    {
        ...
        RolesContainer.AddChild(new PanelContainer { PanelOverride = new StyleBoxFlat { BackgroundColor = color } });
    ...
    {
        var roleCheckbox = new CheckBox { Name = $"{role}RoleCheckbox", Text = role };
        roleCheckbox.OnToggled += args =>
        {
            if (
                args is { Pressed: true, Button.Parent: { } }
                && args.Button.Parent.Children.Where(e => e is CheckBox).All(e => ((CheckBox)e).Pressed)
            )
    ...
    private void OnIpChanged()
    {
        ...
        if (
            ...
            || !byte.TryParse(hid, out var hidInt)
            || hidInt > 128
    ...
    private void OnHwidChanged()
    {
        ...
        if (
            HwidCheckbox.Pressed
            && !(string.IsNullOrEmpty(hwidString) && LastConnCheckbox.Pressed)
            && !ImmutableTypedHwid.TryParse(hwidString, out hwid)
        )
        {
