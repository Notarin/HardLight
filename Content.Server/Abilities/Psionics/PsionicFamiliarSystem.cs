...
public sealed partial class PsionicFamiliarSystem : EntitySystem
{
    ...
    {
        if (
            !familiarComponent.InheritMasterFactions
    ...
    {
        if (!args.FollowMaster || !TryComp<HTNComponent>(familiar, out var htnComponent))
            return;

        _npc.SetBlackboard(
            familiar,
            NPCBlackboard.FollowTarget,
            new EntityCoordinates(master, Vector2.Zero),
            htnComponent
        );
    ...
    {
        if (!args.DoGlimmerEffects || args.MinGlimmer == 0 && args.MaxGlimmer == 0)
            return;

        var minGlimmer = (int)
            Math.Round(
                MathF.MinMagnitude(args.MinGlimmer, args.MaxGlimmer) * component.CurrentAmplification
                    - component.CurrentDampening
            );
        var maxGlimmer = (int)
            Math.Round(
                MathF.MaxMagnitude(args.MinGlimmer, args.MaxGlimmer) * component.CurrentAmplification
                    - component.CurrentDampening
            );
    ...
    {
        if (component.CanAttackMaster || args.Target is null || args.Target != component.Master)
            return;
    ...
    {
        if (!component.DespawnOnFamiliarDeath || args.NewMobState != MobState.Dead)
            return;
