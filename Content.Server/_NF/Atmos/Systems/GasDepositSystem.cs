...
public sealed class GasDepositSystem : SharedGasDepositSystem
{
    ...
    private void OnExtractorUpdate(Entity<GasDepositExtractorComponent> ent, ref AtmosDeviceUpdateEvent args)
    {
        ...
        var allowableMoles =
            (targetPressure - net.Air.Pressure) * net.Air.Volume / (ent.Comp.OutputTemperature * Atmospherics.R);
    ...
    private void OnExtractorRefreshParts(Entity<GasDepositExtractorComponent> ent, ref RefreshPartsEvent args)
    {
        float componentRate;
        ...
        ent.Comp.ExtractionRate =
            ent.Comp.BaseExtractionRate * MathF.Pow(ent.Comp.ExtractionRateMultiplier, componentRate);
    }

    private void OnExtractorUpgradeExamine(Entity<GasDepositExtractorComponent> ent, ref UpgradeExamineEvent args)
    {
        if (ent.Comp.BaseExtractionRate > 0)
            args.AddPercentageUpgrade(
                "gas-deposit-extraction-rate",
                ent.Comp.ExtractionRate / ent.Comp.BaseExtractionRate
            );
    }

    private void OnToggleStatusMessage(
        Entity<GasDepositExtractorComponent> ent,
