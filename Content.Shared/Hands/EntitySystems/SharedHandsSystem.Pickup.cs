...
public abstract partial class SharedHandsSystem : EntitySystem
{
    ...
    )
    {
        ...
        if (animate)
        {
            ...
            if (
                itemPos.MapId == xform.MapID
                && (itemPos.Position - TransformSystem.GetMapCoordinates(uid, xform: xform).Position).Length()
                    <= MaxAnimationRange
                && MetaData(entity).VisibilityMask == MetaData(uid).VisibilityMask
            ) // Don't animate aghost pickups.
            {
    ...
    )
    {
        ...
        if (ContainerSystem.TryGetContainingContainer((entity, null, null), out var container))
        {
            ...
            if (
                _inventory.TryGetSlotEntity(uid, container.ID, out var slotEnt)
                && slotEnt == entity
    ...
    )
    {
        ...
        {
            Log.Error(
                $"Failed to insert {ToPrettyString(entity)} into users hand container when picking up. User: {ToPrettyString(uid)}. Hand: {hand.Name}."
            );
            return;
        }
        ...
        if (log)
        {
            ...
            _adminLogger.Add(
                LogType.Pickup,
                LogImpact.Low,
                $"{ToPrettyString(uid):user} picked up {ToPrettyString(entity):entity}{extraLogs}"
            );
        }
