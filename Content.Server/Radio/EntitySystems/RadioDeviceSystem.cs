...
public sealed class RadioDeviceSystem : EntitySystem
{
    ...
    [Dependency]
    private readonly AccessReaderSystem _access = default!; // Frontier: access

    [Dependency]
    private readonly ActionBlockerSystem _actionBlocker = default!; // Frontier

    [Dependency]
    private readonly SharedPowerReceiverSystem _power = default!; // Goob
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<RadioMicrophoneComponent, SelectHandheldRadioFrequencyMessage>(
            OnChangeHandheldRadioFrequency
        );
        // Nuclear-14-End
    ...
    )
    {
        ...
        if (!quiet && user != null)
        {
            var state = Loc.GetString(
                component.Enabled ? "handheld-radio-component-on-state" : "handheld-radio-component-off-state"
            );
        ...
        using (args.PushGroup(nameof(RadioMicrophoneComponent)))
        {
            args.PushMarkup(
                Loc.GetString(
                    ...
                    (
                        "frequency", /*Nuclear-14-start*/
                        component.Frequency /*Nuclear-14-end*/
                    )
                )
            );
            args.PushMarkup(
                Loc.GetString("handheld-radio-component-chennel-examine", ("channel", proto.LocalizedName))
            );
        }
    }
    ...
    private void OnListen(EntityUid uid, RadioMicrophoneComponent component, ListenEvent args)
    {
        ...
        if (_recentlySent.Add((args.Message, args.Source, channel)))
            _radio.SendRadioMessage(
                ...
                args.Message,
                channel,
                uid, /*Nuclear-14-start*/
                frequency: component.Frequency /*Nuclear-14-end*/
                ,
    ...
    {
        ent.Comp.SupportedChannels = args
            .Component.Channels.Select(p => new ProtoId<RadioChannelPrototype>(p))
            .ToList();
    ...
    private void OnToggleIntercomMic(Entity<IntercomComponent> ent, ref ToggleIntercomMicMessage args)
    {
        if (ent.Comp.RequiresPower && !this.IsPowered(ent, EntityManager))
            return;
        if (!_access.IsAllowed(args.Actor, ent.Owner) || !_actionBlocker.CanComplexInteract(args.Actor)) // Frontier
            return; // Frontier
    ...
    private void OnSelectIntercomChannel(Entity<IntercomComponent> ent, ref SelectIntercomChannelMessage args)
    {
        ...
        if (
            !_protoMan.TryIndex<RadioChannelPrototype>(args.Channel, out var channel)
            || !ent.Comp.SupportedChannels.Contains(args.Channel)
        ) // Nuclear-14: add channel
            return;
    ...
    private void OnBeforeHandheldRadioUiOpen(
        Entity<RadioMicrophoneComponent> microphone,
    ...
    private void OnToggleHandheldRadioMic(
        Entity<RadioMicrophoneComponent> microphone,
        ref ToggleHandheldRadioMicMessage args
    )
    {
        if (!args.Actor.Valid)
            return;
    ...
    private void OnToggleHandheldRadioSpeaker(
        Entity<RadioMicrophoneComponent> microphone,
        ref ToggleHandheldRadioSpeakerMessage args
    )
    {
        if (!args.Actor.Valid)
            return;
    ...
    private void OnChangeHandheldRadioFrequency(
        Entity<RadioMicrophoneComponent> microphone,
        ref SelectHandheldRadioFrequencyMessage args
    )
    {
        if (!args.Actor.Valid)
            return;
    ...
    private void OnGetAltVerbs(EntityUid uid, RadioMicrophoneComponent microphone, GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        if (!_access.IsAllowed(args.User, uid) || !_actionBlocker.CanComplexInteract(args.User))
            return;
