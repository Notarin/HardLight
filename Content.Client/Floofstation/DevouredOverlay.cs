...
// Hardlight - Start
/// <summary>
/// Draws the darkened stomach-wall view for the local player while they have <see cref="DevouredComponent"/>.
/// </summary>
// Hardlight - End
public sealed class DevouredOverlay : Overlay
{
    ...
    protected override bool BeforeDraw(in OverlayDrawArgs args)
    {
        ...
        // Hardlight - Start
        if (playerEntity is not { } player)
            return false;

        var isViewportEye = _entities.TryGetComponent(player, out EyeComponent? eyeComp) &&
                            args.Viewport.Eye == eyeComp.Eye;

        if (!isViewportEye)
            return false;

        return _entities.HasComponent<DevouredComponent>(player);
        // Hardlight - End
    }
