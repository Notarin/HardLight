...
public sealed class PoolManagerTestEventHandler
{
    ...
    private static TimeSpan MaximumTotalTestingTimeLimit
    {
        get
        {
            ...
            if (
                ...
                && double.TryParse(env, out var minutes)
                && minutes > 0
                && minutes < 300
            )
    ...
    public void Setup()
    {
        ...
        _ = Task.Delay(MaximumTotalTestingTimeLimit)
            .ContinueWith(_ =>
            {
        ...
        _ = Task.Delay(HardStopTimeLimit)
            .ContinueWith(_ =>
            {
                var deathReport = PoolManager.DeathReport();
                Environment.FailFast(
                    $"Tests exceeded hard-stop time limit ({HardStopTimeLimit}).\nDeath Report:\n{deathReport}"
                );
            });
    }
