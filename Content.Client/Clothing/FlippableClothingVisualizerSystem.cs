...
public sealed class FlippableClothingVisualizerSystem : VisualizerSystem<FlippableClothingVisualsComponent>
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<FlippableClothingVisualsComponent, GetEquipmentVisualsEvent>(
            OnGetVisuals,
    ...
    {
        if (!TryComp(ent, out SpriteComponent? sprite) || !TryComp(ent, out ClothingComponent? clothing))
            return;

        if (
            clothing.MappedLayer == null
            || !AppearanceSystem.TryGetData<bool>(ent, FoldableSystem.FoldedVisuals.State, out var folding)
            || !SpriteSystem.LayerMapTryGet(
                (ent.Owner, sprite),
