    ...
    public sealed class ParallaxGenerator
    {
        ...
        private sealed class LayerNoise : Layer
        {
            ...
            public LayerNoise(TomlTable table)
            {
                ...
                {
                    SrcFactor = (Color.BlendFactor)
                        Enum.Parse(typeof(Color.BlendFactor), ((TomlValue<string>)tomlObject).Value);
                }

                if (table.TryGetValue("destfactor", out tomlObject))
                {
                    DstFactor = (Color.BlendFactor)
                        Enum.Parse(typeof(Color.BlendFactor), ((TomlValue<string>)tomlObject).Value);
                }
            ...
            public override void Apply(Image<Rgba32> bitmap)
            {
                ...
                for (var y = 0; y < bitmap.Height; y++)
                {
                    for (var x = 0; x < bitmap.Width; x++)
                    {
                        ...
                        var srcColor = Color.InterpolateBetween(OuterColor, InnerColor, noiseVal).WithAlpha(noiseVal);
