...
public sealed class DamageOnAttackedSystem : EntitySystem
{
    ...
    private void OnAttacked(Entity<DamageOnAttackedComponent> entity, ref AttackedEvent args)
    {
        ...
        if (!entity.Comp.IgnoreResistances)
        {
            ...
            _inventorySystem.TryGetInventoryEntity<DamageOnAttackedProtectionComponent>(
                args.User,
            ...
            {
                if (
                    _handsSystem.TryGetActiveItem((args.User, handsComp), out var itemInHand)
        ...
        if (totalDamage != null && totalDamage.AnyPositive())
        {
            _adminLogger.Add(
                LogType.Damaged,
                $"{ToPrettyString(args.User):user} injured themselves by attacking {ToPrettyString(entity):target} and received {totalDamage.GetTotal():damage} damage"
            );
