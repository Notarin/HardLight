...
public sealed class DisposalUnitSystem : SharedDisposalUnitSystem
{
    ...
    {
        if (
            _uiSystem.TryGetOpenUi<DisposalUnitBoundUserInterface>(
                entity.Owner,
                DisposalUnitComponent.DisposalUnitUiKey.Key,
                out var bui
            )
        )
        {
    ...
    {
        if (
            !_appearanceSystem.TryGetData<DisposalUnitComponent.VisualState>(
                ent,
        ...
        _sprite.LayerSetVisible(
            (ent, sprite),
            DisposalUnitVisualLayers.Unanchored,
            state == DisposalUnitComponent.VisualState.UnAnchored
        );
        _sprite.LayerSetVisible(
            (ent, sprite),
            DisposalUnitVisualLayers.Base,
            state == DisposalUnitComponent.VisualState.Anchored
        );
        _sprite.LayerSetVisible(
            (ent, sprite),
            DisposalUnitVisualLayers.OverlayFlush,
            state == DisposalUnitComponent.VisualState.OverlayFlushing
        );
        _sprite.LayerSetVisible(
            (ent, sprite),
            DisposalUnitVisualLayers.BaseCharging,
            state == DisposalUnitComponent.VisualState.OverlayCharging
        );

        var chargingState = _sprite.LayerMapTryGet(
            (ent, sprite),
            DisposalUnitVisualLayers.BaseCharging,
            out var chargingLayer,
            false
        )
        ...
        if (state == DisposalUnitComponent.VisualState.OverlayFlushing)
        {
            ...
            {
                var flushState = _sprite.LayerMapTryGet(
                    (ent, sprite),
                    DisposalUnitVisualLayers.OverlayFlush,
                    out var flushLayer,
                    false
                        ...
                        new AnimationTrackSpriteFlick
                        {
                            ...
                            KeyFrames =
                            {
                                ...
                                new AnimationTrackSpriteFlick.KeyFrame(
                                    chargingState,
                                    (float)ent.Comp.FlushDelay.TotalSeconds
                                ),
                            },
                        },
                    },
        ...
        if (
            !_appearanceSystem.TryGetData<DisposalUnitComponent.HandleState>(
                ent,
        ...
        _sprite.LayerSetVisible(
            (ent, sprite),
            DisposalUnitVisualLayers.OverlayEngaged,
            handleState != DisposalUnitComponent.HandleState.Normal
        );

        if (
            !_appearanceSystem.TryGetData<DisposalUnitComponent.LightStates>(
                ent,
