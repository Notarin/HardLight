...
public sealed class ContrabandPriceGunSystem : EntitySystem
{
    ...
    private void OnUtilityVerb(Entity<ContrabandPriceGunComponent> entity, ref GetVerbsEvent<UtilityVerb> args)
    {
        ...
        if (
            !TryComp<ContrabandComponent>(args.Target, out var contraband)
            || !contraband.TurnInValues.ContainsKey(entity.Comp.Currency)
        )
            return;
        ...
        var verb = new UtilityVerb()
        {
            ...
            Message = Loc.GetString(
                $"{entity.Comp.LocStringPrefix}contraband-price-gun-verb-message",
                ("object", Identity.Entity(args.Target, EntityManager))
            ),
        };
        ...
        if (
            ...
            _popupSystem.PopupEntity(
                Loc.GetString(
                    ...
                    ("price", contraband.TurnInValues[entity.Comp.Currency])
                ),
                args.User,
                args.User
            );
