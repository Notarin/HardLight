...
public abstract class SharedGasThermoMachineSystem : EntitySystem
{
    ...
    private void OnExamined(EntityUid uid, GasThermoMachineComponent thermoMachine, ExaminedEvent args)
    {
        if (
            Loc.TryGetString(
                "gas-thermomachine-system-examined",
                out var str,
    ...
    )
    {
        ...
        _adminLogger.Add(
            ...
            $"{ToPrettyString(args.Actor)} turned {(powerState ? "On" : "Off")} {ToPrettyString(uid)}"
        );
    ...
    )
    {
        ...
        _adminLogger.Add(
            LogType.AtmosTemperatureChanged,
            $"{ToPrettyString(args.Actor)} set temperature on {ToPrettyString(uid)} to {thermoMachine.TargetTemperature}"
        );
