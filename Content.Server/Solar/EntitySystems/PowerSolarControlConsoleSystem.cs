...
namespace Content.Server.Solar.EntitySystems
{
    ...
    {
        [Dependency]
        private readonly NFPowerSolarSystem _powerSolarSystem = default!; // Frontier: use NF variant.
            ...
            if (_updateTimer >= 1)
            {
                ...
                while (query.MoveNext(out var uid, out _, out var uiComp, out var xform))
                {
                    ...
                    if (xform.GridUid != null && TryComp<SolarPoweredGridComponent>(xform.GridUid, out var gridPower))
                        state = new SolarControlConsoleBoundInterfaceState(
                            gridPower.TargetPanelRotation,
                            gridPower.TargetPanelVelocity,
                            gridPower.TotalPanelPower,
                            _powerSolarSystem.TowardsSun
                        );
                    else
        ...
        private void OnUIMessage(
            EntityUid uid,
            SolarControlConsoleComponent component,
            SolarControlConsoleAdjustMessage msg
        )
        {
            ...
            if (double.IsFinite(msg.AngularVelocity))
            {
                ...
                degrees = Math.Clamp(
                    degrees,
                    -PowerSolarSystem.MaxPanelVelocityDegrees,
                    PowerSolarSystem.MaxPanelVelocityDegrees
                );
