...
public sealed class HolopadSystem : SharedHolopadSystem
{
    ...
    private void UpdateHologramSprite(EntityUid hologram, EntityUid? target)
    {
        ...
        if (
            !TryComp<SpriteComponent>(hologram, out var hologramSprite)
        ...
        if (TryComp<SpriteComponent>(target, out var targetSprite))
        {
            ...
            if (TryComp<HolographicAvatarComponent>(target, out var targetAvatar) && targetAvatar.LayerData != null)
            {
        ...
        else
        {
            ...
            var layer = new PrototypeLayerData { RsiPath = holopadhologram.RsiPath, State = holopadhologram.RsiState };
        ...
        {
            if (
                _sprite.TryGetLayer((hologram, hologramSprite), i, out var layer, false)
