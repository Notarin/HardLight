...
public sealed partial class NanoChatUiFragment : BoxContainer
{
    ...
    private void SetupEventHandlers()
    {
        ...
        MessageInput.OnTextChanged += args =>
        {
            ...
            {
                CharacterCount.Text = Loc.GetString(
                    "nano-chat-message-too-long",
                    ("current", length),
    ...
    private void CycleChannel(CycleDirection direction, bool onlyUnread)
    {
        ...
        do
        {
            ...
            onlyUnread && newChatIndex != currentChatIndex && !_recipients[orderedRecipients[newChatIndex]].HasUnread
        );
    ...
    private void SendMessage()
    {
        ...
        var predictedMessage = new NanoChatMessage(_timing.CurTime, messageContent, _ownNumber);
    ...
    {
        if (
            _currentChat is not uint currentChat
    ...
    private void RefreshOpenPopups()
    {
        ...
        {
            if (
                _currentChat is uint currentChat
                && _recipients.TryGetValue(currentChat, out var recipient)
                && recipient.IsGroup
            )
            {
    ...
    private void UpdateCurrentChat()
    {
        ...
        if (hasActiveChat && activeChat != null && _recipients.TryGetValue(activeChat.Value, out recipient))
        {
            ...
            canManageGroup =
                isGroupChat && (recipient.CreatorId == _ownNumber || recipient.Admins?.Contains(_ownNumber) == true);
        }
        ...
        if (hasRecipient) // Funky Station - Changed this so we don't have to TryGetValue twice.
        {
            ...
            else
            {
                CurrentChatName.Text =
                    recipient.Name + (string.IsNullOrEmpty(recipient.JobTitle) ? "" : $" ({recipient.JobTitle})");
            }
        }
        else
        {
