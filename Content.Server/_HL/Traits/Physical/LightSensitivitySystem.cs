...
public sealed class LightSensitivitySystem : EntitySystem
{
    ...
    private SortedDictionary<FixedPoint2, ShadekinState> Thresholds
    {
        get
        {
            ...
            if (
                _proto.TryIndex(ShadekinProto, out var proto)
            ...
            {
                Log.Error(
                    $"Could not read light-exposure thresholds from prototype '{ShadekinProto}'; falling back to default curve."
                );
                _thresholds = new()
                {
    ...
    private void OnRefreshSpeedModifiers(
        EntityUid uid,
        LightSensitivityComponent comp,
