...
public sealed class BeforeLightTargetOverlay : Overlay
{
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        {
            res.EnlargedLightTarget = _clyde.CreateRenderTarget(
                size,
        ...
        args.WorldHandle.RenderInRenderTarget(res.EnlargedLightTarget, () => { }, _clyde.GetClearColor(args.MapUid));
    }

    internal CachedResources GetCachedForViewport(IClydeViewport viewport)
    {
        return _resources.GetForViewport(
            viewport,
            static _ =>
                throw new InvalidOperationException("Expected BeforeLightTargetOverlay to have created its resources")
        );
    }
