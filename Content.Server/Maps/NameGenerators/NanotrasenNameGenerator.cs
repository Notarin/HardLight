...
public sealed partial class NanotrasenNameGenerator : StationNameGenerator
{
    ...
    /// </summary>
    [DataField("prefixCreator")]
    public string PrefixCreator = default!;
    ...
    public override string FormatName(string input)
    {
        ...
        return string.Format(
            input,
            $"{Prefix}{PrefixCreator}",
            $"{random.Pick(SuffixCodes)}-{random.Next(0, 1000):D3}"
        ); // Note: random.Next's max is exclusive, [0-999] = [0,1000)
    }
}
