    ...
    public sealed partial class TabletopChessSetup : TabletopSetup
    {
        ...
        )
        {
            ...
            SpawnPiecesRow(
                ...
                entityManager,
                "White",
                new MapCoordinates(x, y - 7 * separation, mapId),
                separation
            );

            // Extra queens
            EntityUid tempQualifier = entityManager.SpawnEntity(
                "BlackQueen",
                new MapCoordinates(x + 9 * separation + 9f / 32, y - 3 * separation, mapId)
            );
            session.Entities.Add(tempQualifier);
            EntityUid tempQualifier1 = entityManager.SpawnEntity(
                "WhiteQueen",
                new MapCoordinates(x + 9 * separation + 9f / 32, y - 4 * separation, mapId)
            );
            session.Entities.Add(tempQualifier1);
        }

        // TODO: refactor to load FEN instead
        private void SpawnPiecesRow(
            TabletopSession session,
            IEntityManager entityManager,
            string color,
            MapCoordinates left,
            ...
            for (int i = 0; i < 8; i++)
            {
                switch (piecesRow[i])
                {
                    case 'r':
                        EntityUid tempQualifier = entityManager.SpawnEntity(
                            color + "Rook",
                            new MapCoordinates(x + i * separation, y, mapId)
                        );
                        session.Entities.Add(tempQualifier);
                        break;
                    case 'n':
                        ...
                        break;
                    case 'b':
                        EntityUid tempQualifier2 = entityManager.SpawnEntity(
                            color + "Bishop",
                            new MapCoordinates(x + i * separation, y, mapId)
                        );
                        ...
                        break;
                    case 'q':
                        EntityUid tempQualifier3 = entityManager.SpawnEntity(
                            color + "Queen",
                            new MapCoordinates(x + i * separation, y, mapId)
                        );
                        ...
                        break;
                    case 'k':
                        EntityUid tempQualifier4 = entityManager.SpawnEntity(
                            color + "King",
                            new MapCoordinates(x + i * separation, y, mapId)
                        );
