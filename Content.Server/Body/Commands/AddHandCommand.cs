...
namespace Content.Server.Body.Commands
{
    ...
    sealed class AddHandCommand : IConsoleCommand
    {
        ...
        public string Help =>
            $"Usage: {Command} <entityUid> <handPrototypeId> / {Command} <entityUid> / {Command} <handPrototypeId> / {Command}";
        ...
        public void Execute(IConsoleShell shell, string argStr, string[] args)
        {
            ...
            switch (args.Length)
            {
                case 0:
                    ...
                    hand = _entManager.SpawnEntity(
                        DefaultHandPrototype,
                        _entManager.GetComponent<TransformComponent>(entity).Coordinates
                    );
                    break;
                case 1:
                {
            ...
            if (
                !_entManager.TryGetComponent(entity, out BodyComponent? body)
            ...
            {
                shell.WriteError(
                    $"Couldn't create a slot with id {slotId} on entity {_entManager.ToPrettyString(entity)}"
                );
                return;
            }
