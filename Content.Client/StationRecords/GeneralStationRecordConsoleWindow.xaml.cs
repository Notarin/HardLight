...
{
    [Dependency]
    private readonly IPrototypeManager _prototype = default!; // Frontier
    ...
    public GeneralStationRecordConsoleWindow()
    {
        ...
        AdTextBox.Placeholder = new Rope.Leaf(
            Loc.GetString("general-station-record-console-ad-default-text", ("size", MaxAdvertisementLength))
        );
    ...
    private void PopulateJobsContainer(IReadOnlyDictionary<ProtoId<JobPrototype>, int?> jobList)
    {
        ...
        foreach (var (job, amount) in jobList)
        {
            ...
            var jobEntry = new JobRow() { JobName = { Text = jobName }, JobAmount = { Text = amount.ToString() } };
            jobEntry.DecreaseJobSlot.OnPressed += (args) =>
            {
                OnJobSubtract?.Invoke(job);
            };
            jobEntry.IncreaseJobSlot.OnPressed += (args) =>
            {
                OnJobAdd?.Invoke(job);
            };
