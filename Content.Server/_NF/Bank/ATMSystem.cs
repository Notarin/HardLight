...
public sealed partial class BankSystem
{
    ...
    private void OnWithdraw(EntityUid uid, BankATMComponent component, BankWithdrawMessage args)
    {
        ...
        if (!TryComp<BankAccountComponent>(player, out var bank))
        {
            ...
            _uiSystem.SetUiState(uid, args.UiKey, new BankATMMenuInterfaceState(0, false, deposit));
            return;
        }
        ...
        _adminLogger.Add(
            LogType.ATMUsage,
            LogImpact.Low,
            $"{ToPrettyString(player):actor} withdrew {args.Amount} from {ToPrettyString(uid)}"
        );
    ...
    private void OnDeposit(EntityUid uid, BankATMComponent component, BankDepositMessage args)
    {
        ...
        if (
            _prototypeManager.Index<StackPrototype>(component.CashType)
        ...
        _adminLogger.Add(
            LogType.ATMUsage,
            LogImpact.Low,
            $"{ToPrettyString(player):actor} deposited {deposit} into {ToPrettyString(uid)}"
        );
