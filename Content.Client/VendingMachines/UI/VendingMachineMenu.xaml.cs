...
namespace Content.Client.VendingMachines.UI
{
    ...
    public sealed partial class VendingMachineMenu : FancyWindow
    {
        ...
        [Dependency]
        private readonly IComponentFactory _componentFactory = default!; // Frontier
        ...
        public void Populate(
            ...
            int balance,
            int? cashSlotBalance
        ) // Frontier: add priceModifier, balance, cashSlotBalance
        {
            _enabled = enabled;
            _listItems.Clear();
            _amounts.Clear();
            ...
            for (var i = 0; i < inventory.Count; i++)
            {
                ...
                listData.Add(
                    new VendorItemsListData(prototype!.ID, i) // Frontier: prototype<prototype!
                    {
                        ItemText = itemText,
                    }
                );
        ...
        private string GetItemText(EntityUid dummy, uint amount, float priceModifier) // Frontier: add priceModifier
        {
            ...
            if (
                _entityManager.TryGetComponent(dummy, out MetaDataComponent? component)
                && component.EntityPrototype != null
            )
            {
        ...
        {
            SetSize = new Vector2(
                Math.Clamp((longestEntryLength + 2) * 12, 250, 400),
        ...
        private int GetPrototypePrice(EntityPrototype prototype, float priceModifier)
        {
            ...
            double vendPrice = 0;
            if (
                prototype.TryGetComponent<StaticPriceComponent>(out var staticComp, _componentFactory)
                && staticComp.VendPrice > 0.0
            )
            {
                vendPrice += staticComp.VendPrice;
            }
            else if (
                prototype.TryGetComponent<StackPriceComponent>(out var stackComp, _componentFactory)
                && stackComp.VendPrice > 0.0
            )
            {
            ...
            if (prototype.TryGetComponent<StaticPriceComponent>(out var priceComponent, _componentFactory))
            {
                ...
                {
                    if (
                        prototype.TryGetComponent<StackPriceComponent>(out var stackPrice, _componentFactory)
