...
{
    [Dependency]
    private readonly AmbientSoundTreeSystem _treeSys = default!;
    ...
    private static AudioParams _params = AudioParams.Default.WithVariation(0.01f).WithLoop(true).WithMaxDistance(7f);
    ...
    private readonly Dictionary<
        Entity<AmbientSoundComponent>,
        (EntityUid? Stream, SoundSpecifier Sound, string Path)
    > _playingSounds = new();
    ...
    private static bool Callback(ref QueryState state, in ComponentTreeEntry<AmbientSoundComponent> value)
    {
        ...
        var delta =
            xform.ParentUid == state.Player.ParentUid
    ...
    private void ProcessNearbyAmbience(TransformComponent playerXform)
    {
        ...
        foreach (var (ent, sound) in _playingSounds)
        {
            ...
            )
            {
                ...
                var distance =
                    (xform.ParentUid == playerXform.ParentUid)
        ...
        foreach (var (key, sourceList) in state.SourceDict)
        {
            ...
            foreach (var (_, sourceEntity) in sourceList)
            {
                ...
                if (_playingSounds.ContainsKey(sourceEntity) || metaQuery.GetComponent(uid).EntityPaused)
                    continue;
