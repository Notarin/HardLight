...
public static class SharedResearchSystemExtensions
{
    public static int GetTierCompletionPercentage(
        this SharedResearchSystem system,
    ...
    )
    {
        var allTech = prototypeManager
            ...
            .Where(p => system.CanContributeToDisciplineProgress(p, techDiscipline.ID))
            .ToList();
        ...
        var percentage =
            (float)
                component
                    .UnlockedTechnologies.Select(prototypeManager.Index<TechnologyPrototype>)
