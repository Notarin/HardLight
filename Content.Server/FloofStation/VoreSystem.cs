...
public sealed class VoreSystem : EntitySystem
{
    ...
    private bool IsDevourable(EntityUid pred, EntityUid prey)
    {
        if (pred == prey)
            return false;
        if (!_players.TryGetSessionByEntity(pred, out _))
            return false;
        if (!HasComp<BodyComponent>(pred) || !HasComp<BodyComponent>(prey))
            return false;
        if (!IsValidVoreInteraction(pred, prey))
            return false;
        if (!_consent.HasConsent(pred, PredConsent) ||
            (HasComp<ActorComponent>(prey) && !_consent.HasConsent(prey, PreyConsent))) // Hardlight: bypass consent for non-players
            return false;
