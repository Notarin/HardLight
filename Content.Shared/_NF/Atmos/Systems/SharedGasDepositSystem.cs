...
public abstract class SharedGasDepositSystem : EntitySystem
{
    ...
    private void OnExamined(Entity<GasDepositExtractorComponent> ent, ref ExaminedEvent args)
    {
        ...
        args.PushMarkup(
            Loc.GetString(
                "gas-deposit-drill-system-examined",
        ...
        if (_net.IsServer && TryComp(ent.Comp.DepositEntity, out GasDepositComponent? deposit))
        {
            float estimatedAmount =
                MathF.Round(deposit.Deposit.TotalMoles / DrillExamineAmountRound) * DrillExamineAmountRound;
            args.PushMarkup(
                Loc.GetString(
                    "gas-deposit-drill-system-examined-amount",
                    ("statusColor", "lightblue"),
