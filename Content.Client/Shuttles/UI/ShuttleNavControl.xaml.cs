...
public partial class ShuttleNavControl : BaseShuttleControl // Mono
{
    ...
    public ShuttleNavControl()
        : this(64f, 256f, 256f) { } // Mono

    public ShuttleNavControl(float minRange, float maxRange, float range)
        : base(minRange, maxRange, range) // Mono
    {
    ...
    private void EnsureRadarModeButtons()
    {
        ...
        LayoutContainer.SetAnchorAndMarginPreset(
            _radarModeButtons,
            LayoutContainer.LayoutPreset.BottomLeft,
            margin: 10
        );
    }
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        foreach (var grid in _grids)
        {
            ...
            var labelColor = hideColor
                ? blipOnly
                    ? Color.Orange
                    : Color.White
                : _shuttles.GetIFFColor(grid, self: false, iff);
            ...
            if (shouldDrawIFF)
            {
                ...
                if (isOutsideRadarCircle)
                {
                    ...
                    uiPosition = new Vector2(x: uiX + uiXCentre, y: uiY + uiYCentre);
                }
                ...
                if (!isOutsideRadarCircle || isDistantPOI || isMouseOver)
                {
                    ...
                    var displayedDistance =
                        distance < 50f ? $"{distance:0.0}"
                        : distance < 1000 ? $"{distance:0}"
                        : $"{distance / 1000:0.0}k";
                    ...
                    var labelPosition =
                        uiPosition + new Vector2(-labelDimensions.X - blipSize, -labelDimensions.Y * 0.5f) - uiCenter;
                    ...
                    if (
                        !hideLabel
                        && EntManager.TryGetComponent(gUid, out Shared._Mono.Company.CompanyComponent? companyComp)
                    ...
                    if (!hideLabel && lines.Length > 1)
                    {
                        ...
                        handle.DrawString(
                            Font,
                            (uiPosition + companyLabelOffset) * UIScale,
                            companyLabel,
                            UIScale * 0.9f,
                            displayColor
                        );
                    ...
                    if (isMouseOver && !HideCoords)
                    {
                        ...
                        var coordOffset = new Vector2()
                        {
                            ...
                            Y =
                                labelOffset.Y
                                + handle.GetDimensions(Font, mainLabel, 1f).Y
                                + (lines.Length > 1 ? handle.GetDimensions(Font, lines[1], 1f).Y : 0)
                                + 5,
                        };
                        handle.DrawString(
                            Font,
    ...
    protected DetectionLevel GetGridDetected(EntityUid grid)
    {
        ...
        return _consoleEntity == null
            ? DetectionLevel.Undetected
            : _detection.IsGridDetected(grid, _consoleEntity.Value);
    }
        ...
        switch (shape)
        {
            ...
            case RadarBlipShape.Triangle:
                var points = new Vector2[]
                {
                    position
                        + bounds.Origin
                        + bounds.Rotation.RotateVec((bounds.TopLeft + bounds.TopRight) * 0.5f - bounds.Origin),
                    position + bounds.BottomLeft,
                    position + bounds.BottomRight,
                };
    ...
    private void DrawShields(DrawingHandleScreen handle, TransformComponent consoleXform, Matrix3x2 matrix)
    {
        ...
        while (shields.MoveNext(out var uid, out var visuals, out var fixtures, out var xform))
        {
            ...
            var isGhost =
                _playerManager.LocalEntity is { } localEnt && EntManager.HasComponent<GhostComponent>(localEnt);
            ...
            var shieldFixture =
                fixtures.Fixtures.TryGetValue("shield", out var fixture) ? fixture
